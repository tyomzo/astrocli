...
//! a node that refuses to observe the sky because its SD card filled with yesterday's logs has
//! failed the operator worse than one that runs and says so on the console. The failure is
...
//! log line written from a runtime worker blocks that worker on the SD card — precisely the
//! blocking-on-the-runtime failure SDD §2 and PRF-04 exist to prevent.
...
///
/// `log_prefix` names the file (`astroctl-field.log.2026-07-29`); daily rotation keeps one
...
fn default_directive(level: LogLevel) -> String {
    ...
    match level {
        ...
        _ => format!("warn,astroctl_field={ours},astroctl_core={ours},astroctl={ours}"),
    }
}
...
mod tests {
    ...
    fn quiet_levels_keep_third_party_crates_at_warn() {
        ...
        assert!(directive.contains("astroctl_field=info"), "{directive}");
    }
    ...
    fn an_unwritable_log_directory_is_an_error_not_a_panic() {
        ...
        let error = open_file_writer(Path::new("/proc/astroctl-m0t05"), "astroctl-field")
    ...
    fn a_usable_log_directory_opens() {
        ...
        let (_writer, _guard) =
            open_file_writer(&dir, "astroctl-field").expect("a temp directory is writable");
