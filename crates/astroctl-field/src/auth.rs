...
//! * [`require_bearer`] is the **per-request** decision (SEC-02): every route, including WS
//!   upgrades and the `/stack/*` proxy, sits behind it.
//!
//! # Why a startup refusal at all
//!
//! A node with no token bound to `0.0.0.0` is reachable, unauthenticated, from every interface
//! the VPN attaches. Refusing at startup is the earliest point the mistake can be caught: once
//! the node has served one request it is a habit, not an incident. The loopback exception exists
//! so a developer can run the binary with no environment set up at all, and nothing else.
//!
