//! Command line for `astroctl-stack`.
//!
...
pub const DEFAULT_CONFIG_PATH: &str = "/etc/astroctl/stacking-server.yaml";
...
pub const CONFIG_PATH_ENV: &str = "ASTROCTL_STACK_CONFIG";
...
pub enum Invocation {
    ...
    Run {
        /// Path to the stacking-server YAML.
        config: PathBuf,
    },
...
pub const USAGE: &str = "\
astroctl-stack — AstroCtl stacking server (ingest, session mirror, compute workers)

USAGE:
    astroctl-stack [--config <PATH>]

OPTIONS:
    -c, --config <PATH>   Stacking-server configuration file (PRD §8.2).
                          Default: $ASTROCTL_STACK_CONFIG, else /etc/astroctl/stacking-server.yaml
    -h, --help            Print this help and exit
    -V, --version         Print the version and exit

Compute workers are supervised child processes started on demand (SDD §5.12), not at boot.";
    ...
    I: IntoIterator<Item = OsString>,
{
    ...
    while let Some(arg) = args.next() {
        match arg.to_str() {
            ...
            Some("-c" | "--config") => match args.next() {
                Some(path) => config = Some(PathBuf::from(path)),
                None => return Invocation::Invalid(
                    "`--config` needs a path, e.g. `--config /etc/astroctl/stacking-server.yaml`"
                        .to_owned(),
                ),
            },
...
mod tests {
    ...
    fn bad_arguments_are_named_not_ignored() {
        ...
        match parse(args(&["--rebuild-all"]), None) {
            Invocation::Invalid(message) => assert!(message.contains("--rebuild-all"), "{message}"),
