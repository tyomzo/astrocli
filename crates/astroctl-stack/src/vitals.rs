...
/// matches what `df -h` shows the operator when they go and check. The same constant is in
/// `astroctl-field`; the two must agree, because the operator compares the two numbers.
...
/// (both nodes run systemd), not a placeholder — but it is only *one* time source. A node running chrony or ntpd reports `false` here despite having a good clock.
