...
**Date:** 2026-07-29
**Status:** Draft
**Governing documents:** ASTROCTL-PRD-001 v1.18.0, ASTROCTL-ADD-001 v1.5.0, ASTROCTL-SDD-001 v1.19.0
...
**Change note (1.3.0):** M0 gains **M0-T09 (TLS termination on the field node)**, without which USB-09 and USB-10 cannot be met and four of M0-T06's acceptance criteria cannot be honestly run. The M0 exit criterion is qualified accordingly: the container harness and `localhost` both grant a secure context for free, so neither can verify install, wake lock or offline start — those are gated on M0-T09 and a real phone.
...
- TLS termination on the field node (M0-T09): the operator-facing origin is HTTPS, because Chrome gates wake lock, service workers and installability behind a secure context — USB-09 and USB-10 are otherwise unreachable, and a VPN does not substitute for a certificate
- **Exit:** the two containers run, and the PWA loads from the field container showing both nodes' health through the proxy. Development is single-host — containers are how one workstation exercises the two-machine shape honestly, and they make the shaped-link tests CI-runnable. The VPN and the Pi stay real-hardware gates that land with field deployment, not M0 — with one exception now made explicit: the PWA's device capabilities (install, wake lock, offline start) cannot be verified on the container harness or on `localhost`, both of which grant a secure context for free. They are gated on M0-T09 and a real phone.
