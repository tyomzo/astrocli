...
**Governing requirements:** ASTROCTL-PRD-001 v1.20.0
**Change note (1.7.0):** ADR-15 added — fast manual motion as chained bounded firmware gotos, the decision spike E16 forced when the HEQ5 refused every software path to speed (the standing-start stall is invisible to software: the counters count *commanded* steps). Governing-requirements pin advanced to PRD 1.20.0 (MNT-17, `mount.geometry` schema).
...
| ADR-15 | **Fast manual motion is a chain of bounded firmware gotos, not an unbounded slew.** The manual speed ladder splits by mechanism: low classes (≤32×) remain unbounded protocol slews, heard to start; Fast and Max are chains of 30°-bounded goto programs riding the firmware's own acceleration ramp to its cruise (≈51× / ≈835×), re-issued while the button is held, stopped on release, each chunk pre-checked by the travel and collision limits before it is issued | (a) software ramping — start low, re-issue the step period upward on a running slew; (b) a single unbounded high-speed slew; (c) capping manual motion at 32× | Spike E16 measured the wall on the HEQ5: a standing start above ≈32–39× stalls — the counters advance at the commanded rate while the axle stands still, so the failure is *invisible to software* — and the Synta protocol forbids re-rating a running high-speed slew, so (a) buzzes every rung it touches and (b) never starts. Only the firmware's goto path owns a ramp; chaining bounded gotos borrows it. The bound is also the safety property: a lost link or dead client leaves at most one 30° chunk in flight, and every chunk's destination is a checkable predicate (ADR-14) before motion begins. Cost accepted: Max "hold" is a sequence of ramps, not one continuous glide. Hardware-verified 2026-08-03 |
